...
<html>
	<head>
		<title>CardingEmpire-Sign Up Form</title>
		<meta charset="utf-8">
	...
	<body  background="{{asset('images/wp2601081.jpg')}}" style="cursor: url('../icon/cur1162.png'),auto;">
		<center>
			<div id="outPopUp">
				<br><br>				
				<form action="/registration" method="post" name="register">
					...
					<div class="container">
						...
						<h1 style="color: white;"> CardingEmpire </h1>
					  </div>
					  @if(session('message'))
					  ...
					  <div class="mb-3">
					    ...
					    <input type="text" class="form-control {{ $errors->has('username') ? 'border-danger' : '' }}" name="username" id="username"
					    placeholder="Choose username" >
					  </div>
					  <div class="mb-3">
					    <label for="Password" class="form-label">Password*</label>
					    <input type="password" class="form-control {{ $errors->has('password') ? 'border-danger' : '' }}" name="password" id="password" placeholder="Enter your password" onblur="passLength()" required>
					    <span id="passLength"></span>
					  </div>
					  <div class="mb-3">
					    <label for="Password" class="form-label">Retype password*</label>
					    <input type="password" class="form-control" name="password1" id="password1" placeholder="Retype your password" onblur="checkPass()" required>
					    <span id="checkPass"></span>
					  </div>
					  <button type="submit" class="btn btn-primary">Sign up</button>
					  <h6 class="h6">Already Create a account?</h6><div class="register"><a href="/" ><p>Sign in</p></a></div>
				  </div>
				</form>
		...
		function checkPass()
		  {
			  ...
			  var pass2=document.forms["register"]["password1"].value;
		      if (pass1!=pass2)
				{
					document.getElementById('checkPass').style.color = 'red';
					document.getElementById("checkPass").innerHTML="Password mismatched!";
				}
				else
				{
		  ...
		  function passLength()
		  {
		  	...
		  	if(pass1.length<6)
		  	{
				...
				document.getElementById("passLength").innerHTML="Password must be contain 6 character!";
		  	}
		  	else
		  	{
				...
				{
					document.getElementById('passLength').style.color = 'red';
					document.getElementById("passLength").innerHTML="You must need to use one special character";
				}
